export const ApiUrl = Object.freeze({
  USER: '/api/user',
  LOGIN: '/api/user/login',

  MATCHING_POSTS: '/api/matching-posts',
  MATCHING_POST_CAFE_INFO: '/api/matching-posts/cafe-list',
  MATCHING_POST_THEME_INFO: '/api/matching-posts/cafe-infomation',
  MATCHING_POST_READ_POST: '/api/matching-posts/read-post',
  MAP_MATCHING_POSTS: '/api/matching-posts/map',
  MAP_MATCHING_POST_CAFE_INFO: '/api/matching-posts/map/cafepost',

  RECRUIT_INFO: '/api/matching-situation',
  RECRUIT_INFO_EVALUATE: '/api/matching-situation/isEvaluate',
  RECRUIT_LEADER_INFO: '/api/matching-situation/leader',
  RECRUIT_USER_INFO_ALL: '/api/matching-situation/posts',
  RECRUIT_USER_INFO: '/api/matching-situation/post',
  RECRUIT_TEAM_INFO: '/api/matching-situation/myteam',
  RECRUIT_IS_EVALUATE: '/api/matching-situation/isEvaluate',

  ALL_CAFE_DATA: '/api/cafe-infos/all',
  REGION_CAFE_DATA: '/api/cafe-infos/cafeDetail',

  SHORT_EVALUATE_INFO: '/api/evaluate/shortevaluate',
  TEAM_EVALUATE: '/api/evaluate',

  UPLOAD_IMG: '/api/img-upload',
});
